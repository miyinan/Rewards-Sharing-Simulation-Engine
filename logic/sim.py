...
class Simulation(Model):
    ...
    def initialize_agents(self, agent_profile_distr, cost_min, cost_max, pareto_param, stake_distr_source, seed):
        if stake_distr_source == 'file':
            ...
            stake_distribution = hlp.generate_stake_distr_pareto(num_agents=self.n, pareto_param=pareto_param,
                                                                 seed=seed)  # , truncation_factor=self.reward_scheme.k)
    ...
    def append_to_experiment_tracker(self, filename='experiment-tracker.csv'):
        ...
        header = [
            "id", "n", "k", "a0", "phi", "activation order", "reward function",  # "stk distr", "min cost", "max cost",
            "descriptor", "-", "#pools", "#operators", "nakamoto coeff", "comments"
        ]
        row = [
            self.seq_id, self.n, self.reward_scheme.k, self.reward_scheme.a0, self.extra_pool_cost_fraction,
    ...
    def revise_beliefs(self):  # currently not used
        ...
        # Revise expected number of pools, k  (note that the value of global_saturation_threshold, which is used to
        # calculate rewards, does not change in this case)
...
class Ethereum_Sim(Model):
    ...
    def step(self):
        ...
        self.schedule.step()
        if self.current_step_idle:
            self.consecutive_idle_steps += 1 # means a sequence of successive steps where little or no meaningful progress or activity takes place
            if self.has_converged():
                self.equilibrium_steps.append(current_step - self.iterations_after_convergence)
                if self.current_phase < self.total_phases - 1:
                    self.change_phase()
